...
int main(int argc, char *argv[]) {
    ...
    uint32_t  iter = 3;
    while(iter--) {
        for (auto nprobe:{10, 15, 20, 25, 30, 40, 50, 60, 70, 80, 100, 150, 200, 400, 500, 600, 700, 800, 1000, 1500, 2000}) {
            float total_time = 0;
            float total_ratio = 0;
            int correct = 0;
            ...
            fout << recall * 100.0 << " " << (uint32_t) (1e6 / (time_us_per_query))<<" "<<nprobe << endl;
        }
    }
    return 0;
}
